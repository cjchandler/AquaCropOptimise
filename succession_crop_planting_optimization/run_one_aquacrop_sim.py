#this is function to run a single aquacrop sim. The inputs are the greenhouse_climate.csv as df and then a few other parameters 
#the output is much larger than what is collected here, so this can be modified for the parameter of interest
...
def getSimYield( planting_datetime , end_datetime,  df , minimum_harvest_temperature  , crop_file , irrigation_file):
    
    
    print(" crop_file" , crop_file)
    print( " getting sim files ready for year =" , planting_datetime.year , "month = " , planting_datetime.month, "day = " , planting_datetime.day) 
    make_yar_sim_setup_files( planting_datetime.year ,planting_datetime.month , planting_datetime.day ,end_datetime.year , end_datetime.month  ,end_datetime.day , df , crop_file , irrigation_file)#setup the files for sim running
    os.system('./aquacrop')#run the sim 
    ...
    harvest_datetime = datetime( harvest_year , harvest_month , harvest_day)
    # ~ print( harvest_datetime , harvest_year, harvest_month, harvest_day)
    ...
    return yield_fresh_per_ha, harvest_datetime , Tmax_at_start, Tmin_at_start, ndays
